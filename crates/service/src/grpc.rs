...
use ztreamer_protocol::{EncodedCompactBlock, proto, wire};

use crate::{CompactService, service::RpcStream};

// Both adapters share the RPC implementation. The wire adapter substitutes only
// compact-block output types and the methods that prepare them.
macro_rules! impl_grpc {
    ($service:path, $block:ty, $block_method:ident, $range_method:ident) => {
        #[tonic::async_trait]
        impl $service for CompactService {
            type GetBlockRangeStream = RpcStream<$block>;
            type GetBlockRangeNullifiersStream = RpcStream<$block>;
            ...
            async fn get_block(
                ...
                Ok(Response::new(
                    self.$block_method(request.into_inner(), false).await?,
                ))
            }
            ...
            async fn get_block_nullifiers(
                ...
                Ok(Response::new(
                    self.$block_method(request.into_inner(), true).await?,
                ))
            }
            ...
            async fn get_block_range(
                ...
                Ok(Response::new(
                    self.$range_method(request.into_inner(), false).await?,
                ))
            }
            ...
            async fn get_block_range_nullifiers(
                ...
                Ok(Response::new(
                    self.$range_method(request.into_inner(), true).await?,
                ))
            }
...
impl_grpc!(
    proto::compact_tx_streamer_server::CompactTxStreamer,
    proto::CompactBlock,
    block,
    range
);
impl_grpc!(
    wire::compact_tx_streamer_server::CompactTxStreamer,
    EncodedCompactBlock,
    encoded_block,
    encoded_range
);
